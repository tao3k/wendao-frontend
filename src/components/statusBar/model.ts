    ...
    locale === "zh"
      ? issue.phase === "failed"
        ...
        : issue.phase === "unsupported"
          ...
          : issue.phase
      : issue.phase;
  const queueSuffix = issue.queuePosition
    ? locale === "zh"
      ? ` · 排队 #${issue.queuePosition}`
      : ` · Queue #${issue.queuePosition}`
    : "";
...
export function deriveRepoIndexStatusModel(
  ...
  return {
    ...
    concurrencyLabel: repoIndexStatus
  ...
  return {
    label: juliaDeploymentArtifact
      ? formatJuliaArtifactChipLabel(
          ...
          locale as JuliaDeploymentInspectionLocale,
        )
      : null,
