...
describe("VirtualizedSearchResultsList", () => {
  it("keeps a small static-list threshold for measured runtimes and uses an explicit budget above that threshold", () => {
    ...
    expect(
      buildSearchResultsListBudget(SEARCH_RESULTS_STATIC_ROW_LIMIT + 1, "Mozilla/5.0"),
    ).toEqual({
    ...
    expect(
      buildSearchResultsListBudget(SEARCH_RESULTS_STATIC_ROW_LIMIT + 40, "jsdom/22.0.0"),
    ).toEqual({
      shouldUseVirtualizedLayout: false,
    ...
    const manyResultsModel = buildSearchResultsListModel([
      {
        key: "document",
        title: "Documents",
        hits: Array.from({ length: SEARCH_RESULTS_STATIC_ROW_LIMIT + 5 }, (_, index) =>
          buildResult(index),
        ),
      },
    ]);
