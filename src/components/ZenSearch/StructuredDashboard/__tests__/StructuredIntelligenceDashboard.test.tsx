...
describe('StructuredIntelligenceDashboard', () => {
  it('renders the four structured layers and pivots anchors back into query state', () => {
    ...
    expect(screen.getByTestId('structured-dashboard-header').querySelector('.structured-dashboard__header-row')).toBeTruthy();
    expect(screen.getByTestId('structured-dashboard').querySelectorAll('.structured-slot')).toHaveLength(4);
    ...
    expect(screen.getByTestId('structured-topology-map').querySelector('.structured-topology-map__svg')).toHaveAttribute(
      'viewBox',
      '0 0 360 204'
    );
    ...
    fireEvent.click(within(screen.getByTestId('structured-topology-map')).getByRole('button', { name: 'Intro' }));
    ...
    expect(screen.getByTestId('structured-neighbor-kernel/docs/intro.md')).toHaveClass('structured-chip--active');
    expect(screen.getByTestId('structured-dashboard-active-anchor')).toHaveTextContent('Intro');
    expect(screen.getByTestId('structured-dashboard-active-anchor')).toHaveTextContent('kernel/docs/intro.md');
    ...
    expect(within(screen.getByTestId('structured-topology-map')).getByRole('button', { name: 'Appendix' })).toHaveClass(
      'structured-topology-map__node--active'
    );
    ...
    expect(screen.getByTestId('structured-neighbor-kernel/docs/appendix.md')).toHaveClass('structured-chip--active');
    expect(screen.getByTestId('structured-neighbor-kernel/docs/intro.md')).not.toHaveClass('structured-chip--active');
    expect(screen.getByTestId('structured-dashboard-active-anchor')).toHaveTextContent('Appendix');
    expect(screen.getByTestId('structured-dashboard-active-anchor')).toHaveTextContent('kernel/docs/appendix.md');

    fireEvent.click(screen.getByTestId('structured-topology-toggle-incoming'));
    expect(within(screen.getByTestId('structured-topology-map')).queryByRole('button', { name: 'Appendix' })).toBeNull();
    expect(within(screen.getByTestId('structured-topology-map')).getByRole('button', { name: 'Intro' })).toBeInTheDocument();
  ...
  it('renders a compact active-anchor path for deep selections', () => {
    ...
    render(<StructuredIntelligenceDashboard locale="en" preview={preview} onPivotQuery={onPivotQuery} />);

    fireEvent.click(within(screen.getByTestId('structured-topology-map')).getByRole('button', { name: 'Milestone Log' }));
