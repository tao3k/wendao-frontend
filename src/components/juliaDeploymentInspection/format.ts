...
export function formatJuliaArtifactPopoverLines(
  ...
  if (artifact.route) {
    lines.push(locale === 'zh' ? `Flight 路由 ${artifact.route}` : `Flight route ${artifact.route}`);
  }
  if (artifact.healthRoute) {
    lines.push(locale === 'zh' ? `健康检查 ${artifact.healthRoute}` : `Health route ${artifact.healthRoute}`);
  }
  if (typeof artifact.timeoutSecs === 'number') {
    lines.push(locale === 'zh' ? `超时 ${artifact.timeoutSecs}s` : `Timeout ${artifact.timeoutSecs}s`);
  }
