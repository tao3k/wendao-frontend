...
describe("NodeTree", () => {
  const mockTree: TreeNode[] = [
    {
      ...
      name: "Task One",
      type: "task",
      children: [{ id: "node-1-1", name: "Sub Task", type: "task", children: [] }],
    },
    {
      id: "node-2",
      name: "Event Start",
  ...
  it("should call onDoubleClick when node is double-clicked", () => {
    ...
    expect(onDoubleClick).toHaveBeenCalledWith(expect.objectContaining({ id: "node-1" }));
  });
  ...
  it("should show children when node is expanded", () => {
    render(<NodeTree {...defaultProps} expandedNodes={new Set(["node-1"])} />);
  ...
  it("should apply selected class to selected node", () => {
    render(<NodeTree {...defaultProps} selectedNodeId="node-2" />);
