...
export interface FlightWorkspaceTransportDeps {
  ...
  decodeNavigationTarget?: (payload: ArrayBuffer) => StudioNavigationTarget | undefined;
  decodeVfsContent?: (payload: ArrayBuffer) => VfsContentResponse | undefined;
  decodeVfsScan?: (payload: ArrayBuffer, appMetadata: Uint8Array) => VfsScanResult;
}

export function buildWorkspaceFlightDescriptor(
  route: typeof VFS_RESOLVE_ROUTE | typeof VFS_CONTENT_ROUTE | typeof VFS_SCAN_ROUTE,
): FlightDescriptor {
...
export async function loadVfsContentFlight(
  ...
  const content =
    (deps.decodeVfsContent ?? decodeVfsContentResponseFromArrowIpc)(routePayload.payload) ??
...
function decodeVfsContentMetadata(appMetadata: Uint8Array): VfsContentResponse | undefined {
  if (appMetadata.byteLength === 0) {
    return undefined;
  }
  const parsed = JSON.parse(new TextDecoder().decode(appMetadata)) as {
...
function mapFlightWorkspaceError(error: unknown): ApiClientError {
  ...
  if (error instanceof ConnectError) {
    return new ApiClientError(inferFlightWorkspaceErrorCode(error.message), error.message);
  }
  if (error instanceof Error) {
    return new ApiClientError(inferFlightWorkspaceErrorCode(error.message), error.message);
  }
  return new ApiClientError("FLIGHT_WORKSPACE_ERROR", "Unknown Flight workspace failure");
}
