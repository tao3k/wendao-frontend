...
const GITHUB_CONNECT_ADDRESS_REASON_KEY = "failed to connect to github.com: can't assign requested address";
...
const REPO_DIAGNOSTICS_EXPANDED_UNSUPPORTED_REASONS_KEY = 'wendao-repo-diagnostics-expanded-unsupported-reasons';
  ...
  switch (classifyFailureReason(reasonKey)) {
    case 'transient_transport':
      return {
        ...
        retryScope,
        envOverrides: {
          XIUXIAN_WENDAO_REPO_INDEX_SYNC_CONCURRENCY: String(Math.max(1, (syncConcurrencyLimit ?? 2) - 1)),
        },
        followUpChecks: [
...
export function buildUnsupportedManifest(reasons: RepoIndexUnsupportedReason[]): string {
  ...
  for (const reason of reasons) {
    ...
    lines.push(`repos = [${(reason.repoIds ?? []).map((repoId) => JSON.stringify(repoId)).join(', ')}]`);
    if (reason.reason === 'missing Project.toml') {
      lines.push('suggestion = "If docs-only, move these repos to link_graph.projects.* with plugins = []"');
    } else {
