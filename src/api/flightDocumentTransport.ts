...
type DocumentFlightRoute =
  | typeof SEARCH_DEFINITION_ROUTE
  | typeof SEARCH_AUTOCOMPLETE_ROUTE;
...
export function buildDocumentFlightDescriptor(
...
export function buildDefinitionFlightHeaders(
  ...
  if (
    ...
    headers.set(
      ...
      String(Math.floor(request.line))
    );
  }
  return headers;
}

export function buildAutocompleteFlightHeaders(
...
export async function resolveDefinitionFlight(
  ...
  const definitionHits = (
    deps.decodeDefinitionHits ?? decodeDefinitionHitsFromArrowIpc
  ...
  return {
    ...
    ...(typeof metadata.sourcePath === 'string'
      ...
      : {}),
    ...(typeof metadata.sourceLine === 'number'
      ? { sourceLine: metadata.sourceLine }
      : {}),
    ...(navigationTarget ? { navigationTarget } : {}),
    definition,
    candidateCount:
      typeof metadata.candidateCount === 'number'
        ? metadata.candidateCount
        : definitionHits.length,
    selectedScope:
      typeof metadata.selectedScope === 'string'
        ? metadata.selectedScope
        : 'definition',
  };
}
...
export async function searchAutocompleteFlight(
  ...
  const metadata =
    decodeStructuredDocumentMetadata<Partial<AutocompleteResponse>>(
      routePayload.appMetadata
    );
  const suggestions = (
...
function decodeStructuredDocumentMetadata<TMetadata>(
  appMetadata: Uint8Array
): TMetadata {
...
function coerceNavigationTarget(
  value: unknown
...
function mapFlightDocumentError(error: unknown): ApiClientError {
  ...
  if (error instanceof ConnectError) {
    return new ApiClientError(
      inferFlightDocumentErrorCode(error.message),
      error.message,
    );
  }
  ...
  return new ApiClientError(
    'FLIGHT_DOCUMENT_ERROR',
    'Unknown Flight document failure',
  );
}
