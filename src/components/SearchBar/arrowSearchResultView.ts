...
function pushUniqueValues(catalog: SearchFilters, key: keyof SearchFilters, values: string[]): void {
...
function matchesArrowCodeFilters(
  ...
  const hasKindFilter =
    !filters.kind.length || filters.kind.some((value) => kind.includes(value));
  const hasRepoFilter =
    !filters.repo.length || filters.repo.some((value) => repo.includes(value));
  const hasPathFilter =
    !filters.path.length || filters.path.some((value) => path.includes(value));
    ...
    buildCodeFilterCatalog(
      ...
      for (let index = 0; index < table.numRows; index += 1) {
        ...
        if (segments.length > 0) {
          pushUniqueValue(catalog, 'path', segments.slice(0, Math.min(3, segments.length)).join('/'));
        }
