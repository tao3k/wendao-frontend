...
describe('structuredDashboardShared', () => {
  it('compacts long paths and preserves short paths', () => {
    ...
    expect(formatStructuredPath('kernel/docs/deep/section/guide.md')).toBe('kernel/docs/.../guide.md');
  });
  ...
  it('resolves focused anchors from path trail and neighbor lists', () => {
    ...
    expect(resolveFocusedAnchor('kernel/docs/trail.md', centerAnchor, pathTrail, neighbors)).toEqual({
      label: 'Trail',
      value: 'kernel/docs/trail.md',
      query: 'trail-query',
    });
    ...
    expect(resolveFocusedAnchor('missing', centerAnchor, pathTrail, neighbors)).toEqual(centerAnchor);
    expect(resolveFocusedAnchorSide('neighbor-1', centerAnchor, [{ id: 'neighbor-1' }], [])).toBe('incoming');
    expect(resolveFocusedAnchorSide('neighbor-1', centerAnchor, [], [{ id: 'neighbor-1' }])).toBe('outgoing');
    expect(resolveFocusedAnchorSide(centerAnchor.value, centerAnchor, [{ id: 'neighbor-1' }], [])).toBeNull();
  });
});
