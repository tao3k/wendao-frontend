...
describe("RepoDiagnosticsPage", () => {
  ...
  it("restores unsupported filter state from the diagnostics hash", async () => {
    window.location.hash =
      "#repo-diagnostics?filter=unsupported&unsupportedReason=missing%20Project.toml";
  ...
  it("copies a failed repo remediation preset from the detail pane", async () => {
    ...
    expect(copiedText).toContain(
      'follow_up_checks = ["git_credential", "repository_visibility", "remote_url_access"]',
    );
    expect(copiedText).toContain(
  ...
  it("downloads a failure plan for the selected failed repo", async () => {
    ...
    await expect(blob.text()).resolves.toContain(
      'action_key = "verify_git_credentials_and_remote_access"',
    );
  ...
  it("downloads a remediation runbook for the selected failed repo", async () => {
    ...
    await expect(blob.text()).resolves.toContain(
      '# env_overrides = { XIUXIAN_WENDAO_REPO_INDEX_SYNC_CONCURRENCY = "1" }',
    );
    expect(anchorClickMock).toHaveBeenCalledTimes(1);
    expect(revokeObjectUrlMock).toHaveBeenCalledWith("blob:repo-diagnostics");
  });
  ...
  it("downloads a remediation command for the selected failed repo", async () => {
    ...
    await expect(blob.text()).resolves.toContain('# reason_key = "socket_timeout"');
    await expect(blob.text()).resolves.toContain(
      '# action_key = "retry_with_lower_sync_concurrency"',
    );
    await expect(blob.text()).resolves.toContain('# retry_scope = "repo"');
    await expect(blob.text()).resolves.toContain(
      '# env_overrides = { XIUXIAN_WENDAO_REPO_INDEX_SYNC_CONCURRENCY = "1" }',
  ...
  it("renders focused failed-family actions when a failed family is active", async () => {
    ...
    expect(
      within(focusedActions).getByRole("button", { name: "Retry filtered failed" }),
    ).toBeInTheDocument();
    expect(
      within(focusedActions).getByRole("button", { name: "Download current remediation runbook" }),
    ).toBeInTheDocument();
    expect(
      within(focusedActions).getByRole("button", { name: "More exports" }),
    ).toBeInTheDocument();
    expect(
      within(focusedActions).getByRole("button", { name: "Back to all diagnostics" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Copy current remediation command" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Download current failure plan" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Download current remediation command" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders focused unsupported actions when an unsupported slice is active", async () => {
    ...
    expect(
      within(focusedActions).getByRole("button", { name: "Download current config patch" }),
    ).toBeInTheDocument();
    expect(
      within(focusedActions).getByRole("button", { name: "More exports" }),
    ).toBeInTheDocument();
    ...
    expect(
      within(focusedActions).queryByRole("button", { name: "Copy unsupported manifest" }),
    ).not.toBeInTheDocument();
    expect(
      within(focusedActions).queryByRole("button", { name: "Download current triage bundle" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("reveals focused failed-family secondary exports on demand", async () => {
    ...
    const focusedActions = screen.getByTestId("repo-diagnostics-focused-actions");
    expect(
      within(focusedActions).getByRole("button", { name: "Less exports" }),
    ).toBeInTheDocument();
    expect(
      within(focusedActions).getByRole("button", { name: "Copy current remediation command" }),
    ).toBeInTheDocument();
    expect(
      within(focusedActions).getByRole("button", { name: "Download current failure plan" }),
    ).toBeInTheDocument();
    expect(
      within(focusedActions).getByRole("button", { name: "Download current remediation command" }),
    ).toBeInTheDocument();
  });
  ...
  it("reveals focused unsupported secondary exports on demand", async () => {
    ...
    expect(
      within(focusedActions).getByRole("button", { name: "Copy unsupported manifest" }),
    ).toBeInTheDocument();
    expect(
      within(focusedActions).getByRole("button", { name: "Download current triage bundle" }),
    ).toBeInTheDocument();
  });
  ...
  it("downloads the current remediation runbook for failed slices", async () => {
    ...
    await expect(blob.text()).resolves.toContain(
      'affected_repos = ["BipartiteGraphs.jl", "CatalystNetworkAnalysis.jl"]',
    );
    await expect(blob.text()).resolves.toContain("## Failure plan");
    await expect(blob.text()).resolves.toContain("# Repo diagnostics failure plan");
    await expect(blob.text()).resolves.toContain(
      'reason_key = "github_connect_address_unavailable"',
    );
    await expect(blob.text()).resolves.toContain("## Remediation script");
    await expect(blob.text()).resolves.toContain("#!/usr/bin/env bash");
    await expect(blob.text()).resolves.toContain(
  ...
  it("groups repo-specific github transport failures into one failure family plan", async () => {
    ...
    await expect(blob.text()).resolves.toContain(
      "failed to refresh managed mirror `BipartiteGraphs.jl`",
    );
  });
  ...
  it("downloads unsupported presets in the current diagnostics triage bundle for unsupported slices", async () => {
    ...
    await expect(blob.text()).resolves.toContain(
      'alternate_action = "Add Project.toml at the repository root"',
    );
