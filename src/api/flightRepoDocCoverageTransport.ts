...
export function buildRepoDocCoverageFlightHeaders(request: RepoDocCoverageFlightRequest): Headers {
  ...
  if (request.moduleQualifiedName?.trim()) {
    headers.set(WENDAO_REPO_DOC_COVERAGE_MODULE_HEADER, request.moduleQualifiedName.trim());
  }
  return headers;
}
...
export async function loadRepoDocCoverageFlight(
  ...
  try {
    ...
    const metadata = decodeRepoDocCoverageMetadata<Partial<RepoDocCoverageResponse>>(
    ...
    return {
      ...
      ...(typeof metadata.moduleId === "string"
        ...
        : request.moduleQualifiedName
          ...
          : {}),
      coveredSymbols: typeof metadata.coveredSymbols === "number" ? metadata.coveredSymbols : 0,
      uncoveredSymbols:
        typeof metadata.uncoveredSymbols === "number" ? metadata.uncoveredSymbols : 0,
...
function decodeRepoDocCoverageMetadata<TMetadata>(appMetadata: Uint8Array): TMetadata {
...
function missingRepoDocCoverageDecoder(): never {
  throw new Error("loadRepoDocCoverageFlight requires a decodeRepoDocCoverageDocs implementation");
}
...
function mapFlightRepoDocCoverageError(error: unknown): ApiClientError {
  ...
  if (error instanceof ConnectError) {
    return new ApiClientError(inferFlightRepoDocCoverageErrorCode(error.message), error.message);
  }
