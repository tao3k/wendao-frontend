...
export function buildRepoIndexFlightHeaders(request: RepoIndexFlightRequest): Headers {
...
function missingRepoIndexDecoder(): never {
  throw new Error("loadRepoIndexFlight requires a decodeRepoIndexStatusResponse implementation");
}
...
function mapFlightRepoIndexError(error: unknown): ApiClientError {
  ...
  if (error instanceof ConnectError) {
    return new ApiClientError(inferFlightRepoIndexErrorCode(error.message), error.message);
  }
