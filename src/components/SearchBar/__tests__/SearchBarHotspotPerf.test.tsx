...
describe('SearchBar hotspot perf scenarios', () => {
  beforeEach(() => {
    ...
    mockedApi.searchAttachments.mockResolvedValue({ query: '', hits: [], hitCount: 0, selectedScope: 'attachments' });
    mockedApi.searchAst.mockResolvedValue({ query: '', hits: [], hitCount: 0, selectedScope: 'ast' });
    ...
    mockedApi.searchReferences.mockResolvedValue({ query: '', hits: [], hitCount: 0, selectedScope: 'project' });
    mockedApi.searchSymbols.mockResolvedValue({ query: '', hits: [], hitCount: 0, selectedScope: 'project' });
  ...
  it('records the all-scope lang-filter typing hotspot for sec lang:julia', async () => {
    ...
    recordPerfTraceSnapshot(
      'SearchBar hotspot scenario: sec lang:julia',
      snapshot,
    );
  });
  ...
  it('records repo-backed code open hotspot when navigationTarget omits projectName', async () => {
    ...
    mockedApi.searchKnowledge.mockImplementation(async () => {
      trace.increment('search-knowledge-calls');
      return createMockSearchResponse('continuous', [
        {
          stem: 'continuous',
          title: 'continuous',
    ...
    expect(snapshot.renderCount).toBeLessThanOrEqual(6);
    recordPerfTraceSnapshot(
      'SearchBar hotspot scenario: repo-backed code open',
      snapshot,
    );
  });
  ...
  it('records the large-result-list typing hotspot with a bounded render budget', async () => {
    ...
    mockedApi.searchKnowledge.mockImplementation(async (query, _limit, options) => {
      ...
      return createMockSearchResponse(String(query), Array.from({ length: 72 }, (_, index) => ({
        stem: `solve_${index}`,
        title: `solve_${index}`,
    ...
    recordPerfTraceSnapshot(
      'SearchBar hotspot scenario: large code result list typing',
      snapshot,
    );
  });
  ...
  it('records a one-character query refinement from draft input to visible results with bounded refresh churn', async () => {
    ...
    `);
    recordPerfTraceSnapshot(
      'SearchBar hotspot scenario: one-character query refinement',
      snapshot,
    );
  });
  ...
  it('records keyboard browsing inside the suggestion dropdown without opening stale results', async () => {
    ...
    await trace.measureAsync('type-browse-suggestion-into-results', async () => {
      ...
      await act(async () => {
        sectorHybridSearch.resolve(createMockSearchResponse('sector', [
          {
            stem: 'sector_theorem',
    ...
    recordPerfTraceSnapshot(
      'SearchBar hotspot scenario: keyboard suggestion browsing',
      snapshot,
    );
  });
});
