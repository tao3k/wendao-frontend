...
describe('repoDiagnostics state persistence', () => {
  ...
  it('collapses repo-specific github transport failures into one canonical family', () => {
    ...
    expect(failedReasonKey(issues[0])).toBe("failed to connect to github.com: can't assign requested address");
    expect(failedReasonKey(issues[1])).toBe("failed to connect to github.com: can't assign requested address");
